...
public abstract class GameEntity {
    ...
    public float getHeight() {
        /*
        Quick fix: if this entity uses animationSpec instead of VisualSpec and PhysicsSpec, return the YExtent from
        EntityFactory.build().
         */
        return type.blueprint().visual() == null ? ((BoundingBox)characterNode.getWorldBound()).getYExtent()*2 : type.blueprint().visual().height();
    }

    public PhysicsControl getControl(){
        String controlType = type.blueprint().physics() == null ? BodyKind.CHARACTER.getControlString() : type.blueprint().physics().body().getControlString();
