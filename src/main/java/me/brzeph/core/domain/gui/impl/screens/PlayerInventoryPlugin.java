...
import me.brzeph.core.domain.gui.core.others.Align;
...
public final class PlayerInventoryPlugin implements ScreenPlugin {
    ...
    public Screen build(ScreenContext ctx, ScreenParams params) {
        ...
        int   goldParam = params.has("gold")      ? params.get("gold", Integer.class)  : -1;

        // Player / Inventário (fonte de dados)
        var player = ((PlayerSystem)SystemAbs.getSystem(PlayerSystem.class)).getPlayer();
        InventoryPort port = player.getInventory();
        int gold = (goldParam >= 0) ? goldParam : port.goldAmount();
        ...
        UIMeter peso  = new UIMeter().value(weight / Math.max(1f, maxWeight))
                .height(12f)
                .label(String.format("Peso: %.0f / %.0f", weight, maxWeight));
        ...
        Panel equipCard = new Panel()
                ...
                .add(InventoryAdapter.makeEquipmentPanel(PLAYER_INVENTORY, port, /*vgap*/8f, /*hgap*/6f)
                        .layout(LayoutParams.wrap()));

        // ----------------- MOCHILA (card com grid comum) -----------------
        UIGrid commonGrid = new UIGrid().layout(LayoutParams.flex(1f));
        InventoryAdapter.fillCommonGrid(commonGrid, port, PLAYER_INVENTORY, 6, 6f);
